CREATE DATABASE  IF NOT EXISTS `hrdb` /*!40100 DEFAULT CHARACTER SET utf8 COLLATE utf8_polish_ci */;
USE `hrdb`;
...
CREATE TABLE `jobs` (
    `job_id` VARCHAR(10) COLLATE UTF8_POLISH_CI NOT NULL DEFAULT '',
    `job_title` VARCHAR(35) COLLATE UTF8_POLISH_CI NOT NULL,
...
CREATE TABLE `regions` (
    ...
    `region_name` CHAR(25) COLLATE UTF8_POLISH_CI DEFAULT NULL,
    PRIMARY KEY (`region_id`)
)  ENGINE=INNODB DEFAULT CHARSET=UTF8 COLLATE = UTF8_POLISH_CI;

CREATE TABLE `countries` (
    `country_id` VARCHAR(2) COLLATE UTF8_POLISH_CI DEFAULT NULL,
    `country_name` VARCHAR(40) COLLATE UTF8_POLISH_CI DEFAULT NULL,
    `region_id` DECIMAL(10 , 0 ) DEFAULT NULL,
    PRIMARY KEY (`country_id`),
    CONSTRAINT `fk_region_id_countries` FOREIGN KEY (`region_id`) REFERENCES `regions` (`region_id`) ON DELETE CASCADE ON UPDATE CASCADE
)  ENGINE=INNODB DEFAULT CHARSET=UTF8 COLLATE = UTF8_POLISH_CI;

CREATE TABLE `locations` (
    `location_id` DECIMAL(4 , 0 ) NOT NULL DEFAULT '0',
    `street_address` VARCHAR(40) COLLATE UTF8_POLISH_CI DEFAULT NULL,
    `postal_code` VARCHAR(12) COLLATE UTF8_POLISH_CI DEFAULT NULL,
    `city` VARCHAR(30) COLLATE UTF8_POLISH_CI NOT NULL,
    `state_province` VARCHAR(25) COLLATE UTF8_POLISH_CI DEFAULT NULL,
    `country_id` VARCHAR(2) COLLATE UTF8_POLISH_CI DEFAULT NULL,
    PRIMARY KEY (`location_id`),
    CONSTRAINT `fk_country_id_locations` FOREIGN KEY (`country_id`) REFERENCES `countries` (`country_id`) ON DELETE CASCADE ON UPDATE CASCADE
)  ENGINE=INNODB DEFAULT CHARSET=UTF8 COLLATE = UTF8_POLISH_CI;

CREATE TABLE `departments` (
    `department_id` DECIMAL(4 , 0 ) NOT NULL,
    `department_name` VARCHAR(30) COLLATE UTF8_POLISH_CI NOT NULL,
    `manager_id` DECIMAL(6 , 0 ) DEFAULT NULL,
    `location_id` DECIMAL(4 , 0 ) DEFAULT NULL,
    PRIMARY KEY (`department_id`),
    KEY `fk_location_id` (`location_id`),
    CONSTRAINT `fk_location_id_departments` FOREIGN KEY (`location_id`) REFERENCES `locations` (`location_id`) ON DELETE CASCADE ON UPDATE CASCADE
)  ENGINE=INNODB DEFAULT CHARSET=UTF8 COLLATE = UTF8_POLISH_CI;

CREATE TABLE `employees` (
    `employee_id` DECIMAL(6 , 0 ) NOT NULL DEFAULT '0',
    `first_name` VARCHAR(20) COLLATE UTF8_POLISH_CI DEFAULT NULL,
    ...
    `job_id` VARCHAR(10) COLLATE UTF8_POLISH_CI NOT NULL,
    `salary` DECIMAL(8 , 2 ) DEFAULT NULL,
    `commission_pct` DECIMAL(2 , 2 ) DEFAULT NULL,
    `manager_id` DECIMAL(6 , 0 ) DEFAULT NULL,
    `department_id` DECIMAL(4 , 0 ) DEFAULT NULL,
    PRIMARY KEY (`employee_id`),
	CONSTRAINT `fk_job_id_employees` FOREIGN KEY (`job_id`) REFERENCES `jobs` (`job_id`) ON DELETE CASCADE ON UPDATE CASCADE,
    CONSTRAINT `fk_department_id_employees` FOREIGN KEY (`department_id`) REFERENCES `departments` (`department_id`) ON DELETE CASCADE ON UPDATE CASCADE
)  ENGINE=INNODB DEFAULT CHARSET=UTF8 COLLATE = UTF8_POLISH_CI;

CREATE TABLE `job_grades` (
    `grade_level` VARCHAR(20) COLLATE UTF8_POLISH_CI NOT NULL,
    `lowest_sal` DECIMAL(5 , 0 ) NOT NULL,
    `highest_sal` DECIMAL(5 , 0 ) NOT NULL,
...
CREATE TABLE `job_history` (
	...
	CONSTRAINT `fk_employee_id_jh` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`employee_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_job_id_jh` FOREIGN KEY (`job_id`) REFERENCES `jobs` (`job_id`) ON DELETE CASCADE ON UPDATE CASCADE ,
    CONSTRAINT `fk_department_id_jh` FOREIGN KEY (`department_id`) REFERENCES `departments` (`department_id`) ON DELETE CASCADE ON UPDATE CASCADE
)  ENGINE=INNODB DEFAULT CHARSET=UTF8 COLLATE = UTF8_POLISH_CI;
